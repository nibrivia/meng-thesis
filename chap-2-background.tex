...
We will see an example of such a reformulation when in section \ref{} when we talk about timeouts. % TODO write section, model chapter?
% Typically cases where events may be sent out of order comes from different events, can create multiple channels
...
This handling of ties will turn out to be an important optimization when we talk about scheduling in section \ref{rustasim-sched}. % TODO make sure it is talked about

\subsection{Making progress with null-message passing} \label{null-messages}
...
The loop optimizations are essentially identical to the stall technique described above. % TODO double check
